...
library(ggplot2)
tema_taller <- theme_minimal(base_size = 12) +
  theme(
...
Primero buscamos cada uno de los $n$,$c$ y $r$ para cada uno de los
planes simples (1,2,3) en cada uno de los niveles, por medio de la
función trabajada en clase:
...
Representamos los n, r y c según su clasificación de nivel en la
siguente tabla (Table 1).
...
valores de aceptación 𝑐 son más pequeños que en los otros tipos de
...
muestreo simple, la cual es que los planes estrictos priorizan la
protección del consumidor, los planes reducidos favorecen la eficiencia
para el productor cuando existe confianza en la calidad del proceso, y
los planes normales mantienen un balance entre ambos intereses.
Asimismo, el aumento en el nivel del plan implica un mayor tamaño de
muestra y, por lo tanto, una mayor capacidad para discriminar entre
diferentes niveles de calidad en los lotes inspeccionados.
...
Se replicará la metodología del plan de muestreo simple para este
esquema de muestreo doble. Los valores han sido calculados mediante las
funciones correspondientes, con excepción del plan doble reducido, cuyos
parámetros se han predefinido para los fines académicos de este
ejercicio:
...
En síntesis, los planes normales, y en particular el nivel II,
representan el punto de equilibrio clásico que la norma MIL-STD-105E
propone como estándar. Combinan una capacidad de discriminación
aceptable con un esfuerzo de inspección moderado, lo que los convierte
en la opción más versátil y utilizada en la práctica. En definitiva, la
elección entre uno u otro tipo de plan dependerá del balance que se
quiera lograr entre la protección al consumidor, los costos de
inspección y la confianza depositada en el proveedor, siendo las curvas
OC y AOQ herramientas fundamentales para tomar esa decisión con criterio
técnico.

## c. Comparación del mejor plan simple vs. el mejor plan doble
...
la cual es similar, sino en cómo se distribuye el esfuerzo de muestreo
para lograr esa discriminación, aspecto que se revela con mayor claridad
en la curva ASN.

```{r grafica aoq c, echo=FALSE}
...
Respecto al número promedio total inspeccionado (ATI), el plan de
muestreo doble puede resultar más eficiente en ciertos casos, ya que
permite tomar decisiones tempranas después de la primera muestra cuando
la calidad del lote es claramente buena o claramente mala. Esto puede
reducir el esfuerzo total de inspección en comparación con el plan
simple.

En conclusión, ambos planes presentan comportamientos estadísticos
similares en términos de OC y AOQ, lo que indica niveles de protección
comparables frente a la calidad del lote. No obstante, el plan doble
puede ofrecer una ventaja en términos de eficiencia de inspección debido
a la posibilidad de tomar decisiones con una primera muestra. Por otro
lado, el plan simple presenta una aplicación operativa más directa al
requerir siempre el mismo tamaño de muestra. Por lo tanto, la elección
entre ambos dependerá del equilibrio deseado entre simplicidad operativa
y eficiencia en el esfuerzo de inspección.
